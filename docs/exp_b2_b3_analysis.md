...
**缓存层延迟配置：**
- **L1访问延迟**：15ns（NIC本地SRAM，最快）
- **CAS操作延迟**：5ns（原子操作基础延迟）
...
2. **延迟影响**：
   - 低并发时延迟主要由L1访问延迟（15ns）决定
   - 高并发时延迟主要由CAS重试和Fallback开销决定
   - 延迟曲线呈现非线性增长
...
**缓存层延迟配置：**
- **L1访问延迟**：15ns（NIC本地SRAM）
- **L2访问延迟**：80ns（CXL内存）
- **L3访问延迟**：800ns（Host DRAM，通过PCIe）

**DMA配置：**
- **DMA设置延迟**：100ns
- **DMA带宽**：32,000 MB/s（模拟CXL带宽）
